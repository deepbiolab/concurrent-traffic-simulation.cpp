...
在这一点上，你应该已经对基本线程、任务管理以及共享数据保护有了扎实的理解。我们已经看到，**数据保护是并发编程中的一个关键要素**。在研究了实现这一目标的几种方法之后，我们现在希望在这些概念的基础上更进一步。

现在，这里的**目标是开发一种方法，用于多个线程之间受控且最终范围的数据交换**。在本课中，我们将构建一个并发消息队列。这意味着你需要理解一种数据交换协议，它可以用于大量需要线程高效且安全通信的场景。然而，在我们构建消息队列之前，你首先需要理解一些**基本概念，比如监视器对象模式和条件变量**。

## The Monitor Object Pattern

在前面的章节中，我们了解到数据保护是并发编程中的一个关键要素。在研究了实现这一目标的几种方法之后，我们现在希望在这些概念的基础上，设计一种用于**线程之间受控且精细化数据交换**的方法——并发消息队列。实现这种结构的一个**重要步骤是实现一个监视器对象**，这是一种设计模式，用于同步并发方法的执行，**以确保在一个对象中一次只能运行一个方法**。它**还允许对象的方法协作安排它们的执行顺序**。**该模式解决的问题基于以下观察：许多应用程序包含对象，这些对象的方法会被多个客户端线程并发调用**。

这些方法通常会修改对象的状态，**例如向内部向量中添加数据**。为了使这样的并发程序能够正确执行，必须非常谨慎地同步和安排对这些对象的访问。**监视器对象的思想是同步对对象方法的访问，以确保在任何时候只能执行一个方法**。

<img src="assets/CleanShot 2025-01-02 at 21.46.16@2x.png" alt="CleanShot 2025-01-02 at 21.46.16@2x" style="zoom:50%;" />
...
在之前的章节中，我们看过一个代码示例，它的功能非常接近监视器对象：类`WaitingVehicles`。

让我们修改并部分重新实现这个类，我们希望将其用作一个共享的地方，供并发线程存储数据，在我们的例子中是类`Vehicle`的实例。由于我们将为所有线程使用相同的`WaitingVehicles`对象，因此我们必须**通过引用将其传递给线程**——并且由于所有线程将同时对该对象进行写操作（这是一个变更操作），我们将其作为共享指针传递。

```cpp
    std::shared_ptr<WaitingVehicles> queue(new WaitingVehicles);
```

请记住，会有许多线程试图同时向`WaitingVehicles`对象传递数据，因此存在数据竞争的风险。

在查看`WaitingVehicles`的实现之前，让我们先看一下主函数，在那里所有线程都会被创建。我们需要一个向量来存储期望值（futures），因为线程不会返回任何数据。

```cpp
    std::vector<std::future<void>> futures;
```

此外，我们需要在`main()`的末尾调用`wait()`方法，这样程序不会在线程执行完成之前过早退出。
...
我们将再次使用`emplace_back`，而不是`push_back`，以便直接在向量中构造期望值（`futures`），而不是将其移动到向量中。在for循环中构造一个新的`Vehicle`对象后，我们通过传递对`pushBack`函数的引用、一个指向`WaitingVehicles`对象的共享指针以及新创建的车辆对象来启动一个新任务。注意，最后一个参数是通过移动语义传递的。
...
现在让我们来看一下`WaitingVehicle`对象的实现。

我们需要让它能够同时处理来自多个线程的写请求。每当线程发出请求时，该对象需要将新数据添加到其内部存储中。我们的存储容器将使用`std::vector`。

```cpp
    std::vector<Vehicle> _vehicles; // list of all vehicles waiting to enter this intersection
```

由于我们需要保护该向量免受同时访问的影响，因此还需要在**类中**集成一个互斥锁（mutex）。

```cpp
    std::mutex _mutex;
```

正如我们已经知道的，互斥锁具有lock和unlock方法。为了避免数据竞争，每次线程想要访问向量时， 都需要锁定互斥锁，并且在写操作完成后需要解锁互斥锁。**为了避免由于缺少解锁操作而导致程序冻结，我们将使用一个锁守卫（lock guard）对象**，它会在锁对象超出作用域时自动解锁。

```cpp
        std::lock_guard<std::mutex> myLock(_mutex); // lock is released when myLock goes out of scope
```

在我们修改后的pushBack函数中，我们首先创建一个锁守卫对象，并将互斥锁成员变量传递给它。现在，我们可以安全地将Vehicle对象移动到向量中，而无需担心数据竞争。
...
在函数的末尾，有一个对std::sleep_for的调用，它**模拟了一些工作，并帮助我们更好地暴露潜在的并发问题**。每当一个新的Vehicle对象被传递到队列中时，我们都会在控制台上看到一个输出。

`WaitingVehicle`类中的另一个函数是`printIDs()`，它会遍历向量中的所有元素并将它们各自的ID打印到控制台。
...
`pushBack()`和`printIDs()`之间的一个主要区别在于，后者通过遍历向量访问所有的`Vehicle`对象，而`pushBack`只访问单个对象——也就是最近添加到`Vehicle`向量中的对象。

当程序运行后，会有下面的输出结果

![C4-4-A2](assets/C4-4-A2.gif)

正如可以看到的，`Vehicle`对象是逐个添加的，所有线程都会按顺序等待它们的执行机会。然后，一旦所有`Vehicle`对象都被存储，调用`printIDs`会一次性打印`vector`的全部内容。

虽然我们构建的`monitor`对象在功能上相比许多其他允许线程传递数据的方法有所改进，但它有一个显著的缺点：**主线程必须等待所有工作线程完成它们的任务，然后才能批量访问添加的数据**。然而，一个**真正交互式的系统**需要能够在事件到达时立即做出反应——它不应该等到所有线程完成任务后再行动，而是应该在新数据到达时立刻采取行动。在接下来的内容中，我们希望为我们的`monitor`对象添加这一功能。

### 创建一个无限轮询循环

当线程使用`pushBack`方法将数据逐步添加到`monitor`时，主线程会在最后使用`printSize`一次性显示所有结果。我们的目标是修改代码，使主线程在每次有新数据可用时都能收到通知。但是，主线程如何知道是否有新数据可用呢？**解决方案是编写一个新方法，定期检查新数据是否到达。**

在下面列出的代码中，添加了一个新方法`dataIsAvailable()`，而`printIDs()`方法被移除了。这个方法在`vector`中有数据时返回`true`，否则返回`false`。一旦主线程通过`dataIsAvailable()`发现`vector`中有新数据，就可以调用`popBack()`方法从`monitor`对象中检索数据。需要注意的是，数据不是被复制，而是从`vector`中移动到主方法中。
...
在主线程中，我们将使用一个**无限的`while`循环来频繁地轮询`monitor`对象**，检查是否有新数据到达。与之前不同的是，我们现在将在工作线程完成之前执行读取操作——因此我们必须在主函数末尾调用`futures`的`wait()`之前集成这个循环。一旦有新的`Vehicle`对象可用，我们希望在循环中打印它。

执行代码时，我们会看到类似于下面列出的控制台输出：

![DraggedImage](assets/DraggedImage.png)

从输出中可以很容易地看出，向`monitor`对象添加数据和从中移除数据现在是交错进行的。当多次执行代码时，车辆的顺序很可能会在不同的执行中有所不同。



### Quiz: 编写一个车辆计数器  

请注意，上述示例中的程序并未终止——即使没有新的`Vehicle`对象被添加到队列中，无限的`while`循环也不会退出。

解决此问题的一种可能方法是，在`WaitingVehicles`类中集成一个车辆计数器。每次添加一个`Vehicle`对象时，该计数器会递增；每次移除一个`Vehicle`对象时，计数器会递减。当计数器达到零时，`while`循环即可终止。请继续实现此功能——但请记住，**需要保护计数器**，因为它将被多个线程同时访问。此外，在生成线程和收集结果之间引入一个小的延迟是个好主意。否则，队列默认会是空的，程序将过早终止。在`main()`函数的末尾，请打印`vector`中剩余的`Vehicle`对象数量。
...
**但是，我们实现的无限轮询循环并不是最优的**。只要程序在运行，`while`循环就会让处理器完全处于忙碌状态，不断地检查是否有新数据可用.

## Building a Concurrent Message Queue

现在，这是开始最终项目之前的最后一个部分。在上一部分中，我们使用的**无限轮询循环**由于以非常短的时间间隔不断迭代，**给处理器带来了很大的负载**。**替代无限轮询的方法**是阻塞、进入睡眠状态，并等待一个信号通知我们有新数据可用，然后再唤醒并获取数据。

在之前的某个部分中，我们已经讨论过`promise`和`future`的概念，它们原则上可以完成这个任务。然而，`future`的一个问题是它只能使用一次，而对于我们的消息队列来说，我们需要一种可以根据需要重复使用的信号机制。这就是为什么我们将研究**`condition variable`**，它是我们目前缺少的最后一个构建块之一。

在本节的第二部分，我们将开始实际实现一个**并发消息队列**。



### 条件变量

在之前的示例中，我们使用的轮询循环并不是最优的：只要程序在运行，`while`循环就会让处理器一直处于忙碌状态，不断检查是否有新数据可用。接下来，我们将探讨一种更好的方法来解决这个问题，同时避免对处理器造成过大的负载。

**替代轮询循环的方法**是让主线程阻塞并等待一个信号，通知我们有新数据可用。这可以防止无限循环持续占用处理器资源。我们之前已经讨论过一个可以实现这一目的的机制——`promise-future`构造。然而，`future`的一个问题是它只能使用一次。一旦`future`变为就绪状态并调用了`get()`，它就不能再被使用了。而对于我们的需求，我们需要一种可以重复使用的信号机制。C++标准库为此提供了一种构造：**`std::condition_variable`**。

### `std::condition_variable` 的基本原理

`std::condition_variable` 提供了一个方法 `wait()`，当线程调用它时会阻塞。条件变量会保持阻塞状态，直到另一个线程通过调用`notify_one()`或`notify_all()`方法将其释放。这两个方法的关键区别在于：  

- **`notify_one()`** 只会唤醒一个正在等待的线程。  
- **`notify_all()`** 会一次性唤醒所有正在等待的线程。

条件变量是更高级通信协议的低级构建块。它本身既不具备存储功能，也不会记住通知。例如，如果一个线程在另一个线程调用`notify()`之前调用了`wait()`，条件变量会按预期工作，前一个线程会被唤醒。然而，如果调用顺序反过来，先调用了`notify()`再调用`wait()`，那么通知会丢失，线程将无限期阻塞。因此，在更复杂的通信协议中，条件变量通常需要与另一个可以独立检查的共享状态配合使用。在这种情况下，通知条件变量仅意味着继续检查这个共享状态。

### 两种场景下的协议

假设我们有一个共享变量`dataIsAvailable`，它是一个布尔值。接下来，我们讨论两种场景，分别由生产者线程和消费者线程主导。

#### 场景 1：消费者线程先行动

![C4-4-A5a](assets/C4-4-A5a.png)

消费者线程检查`dataIsAvailable`，发现其值为`false`，于是消费者线程阻塞并`wait`条件变量。随后，生产者线程将`dataIsAvailable`设置为`true`，并调用条件变量的`notify_one()`。此时，消费者线程被唤醒并继续工作。

#### 场景 2：生产者线程先行动

![C4-4-A5b](assets/C4-4-A5b.png)

生产者线程首先将`dataIsAvailable`设置为`true`，然后调用`notify_one()`。随后，消费者线程检查`dataIsAvailable`，发现其值为`true`，因此不调用`wait()`，直接继续工作。即使通知丢失，这种构造也不会出现问题——消息已经通过`dataIsAvailable`成功传递，并且避免了等待锁的情况。

### 并发编程中的复杂性

在理想的（非并发）环境下，上述两种场景可能已经足够描述所有可能的组合。然而，在并发编程中，事情并没有那么简单。如图所示，有四个原子操作，每个线程各有两个操作。在足够多的执行次数下，所有可能的操作交错都会出现——我们需要找到那些可能导致问题的交错组合。

#### 一个可能导致锁死的组合

![C4-4-A5c](assets/C4-4-A5c.png)

消费者线程读取`dataIsAvailable`，发现其值为`false`。随后，生产者线程将`dataIsAvailable`设置为`true`并调用`notify()`。由于操作交错的不幸组合，消费者线程在看到`dataIsAvailable`为`false`后调用了`wait()`。这是可能的，因为消费者线程的任务并不是一个联合的原子操作，而是**可能被调度器分离**，并与其他任务**交错执行**——在本例中是生产者线程的两个操作

问题在于，消费者线程调用`wait()`后将永远无法再次被唤醒。此外，正如你可能已经注意到的，共享变量`dataIsAvailable`在这里并没有被互斥量保护，这使得问题更容易发生。

#### 一个潜在的解决方案

一个快速想到的解决方案可能是将`dataIsAvailable`的检查和`wait()`操作放在一个加锁的互斥量中执行。虽然这确实可以有效防止不同线程任务的交错，但它也会阻止其他线程对`dataIsAvailable`进行修改。

讨论这些失败场景的一个原因是让你意识到并发行为的复杂性——即使是像我们现在讨论的这样简单的协议。

### 最终解决方案

我们来看解决上述问题的最终方案，以及一个可行的通信协议版本。

![C4-4-A5d](assets/C4-4-A5d.png)

如图所示，我们通过在加锁状态下读取共享状态并进入`wait()`来弥补状态读取和进入等待之间的间隙。我们在加锁状态下读取状态（红色条表示），并在加锁状态下调用`wait()`。然后，`wait()`会在一个原子步骤中释放锁并进入等待状态。这之所以可行，是因为`wait()`方法可以接受一个锁作为参数。然而，传递给`wait()`的锁不能是我们之前常用的`lock_guard`，而必须是一个可以在`wait()`内部临时解锁的锁——适合这种用途的锁是我们在上一节中讨论过的`unique_lock`类型。

### 实现 `WaitingVehicles` 队列

现在我们已经掌握了实现并发队列所需的所有内容，可以按照上图的流程开始实现用于存储等待中`Vehicle`对象的并发队列。

第一步是将一个`condition_variable`添加到`WaitingVehicles`类中，作为一个私有成员——就像`mutex`一样。
...
接下来，在将一个新的`Vehicle`对象推入`vector`后，我们需要通知客户端。
...
在`popBack`方法中，我们需要先创建一个锁——它不能再是`lock_guard`，因为我们需要将它传递给条件变量的`wait`方法。因此，它必须是一个`unique_lock`。

现在我们可以进入等待状态，同时释放锁。需要特别注意的是：**只有在`wait`方法内部，互斥量才会被临时解锁**——这是一个非常重要的点。我们在调用`wait`之前和之后都持有锁——这意味着我们可以安全地访问由互斥量保护的数据。在我们的例子中，这将是`dataIsAvailable()`。

在继续之前，我们需要讨论一个问题：**“伪唤醒”（spurious wake-ups）**。系统偶尔会无缘无故地唤醒一个线程。如果在没有采取适当预防措施的情况下发生这种伪唤醒，我们可能会调用`wait`，而实际上并没有新数据可用（因为这种唤醒并不是由条件变量引起的，而是由系统引起的）。为了防止在这种情况下调用`wait`，我们需要稍微修改代码：

![C4-4-A6c](assets/C4-4-A6c.png)

在这段代码中，即使发生了伪唤醒，我们也会检查数据是否真的可用。如果数据可用，我们将调用条件变量的`wait`方法。而且，只有在`wait`内部，其他线程才可以修改和访问`dataIsAvailable`。

如果`vector`为空，则会调用`wait`。当线程再次被唤醒时，会立即重新检查条件——如果唤醒不是伪唤醒，我们可以继续工作并检索`vector`中的数据。

我们可以进一步简化这段代码，让`wait()`函数同时完成测试和循环的工作。我们可以直接将一个Lambda表达式传递给`wait()`，它会反复检查`vector`中是否包含元素（因此逻辑表达式是取反的）：
...
当`wait()`完成时，我们可以保证此时`vector`中一定有新元素。而且，我们仍然持有锁，因此没有其他线程可以访问`vector`——在这种情况下不存在数据竞争的风险。一旦超出了作用域，锁会自动释放。

在`main()`函数中，尽管仍然有一个无限轮询循环来查询是否有新的`Vehicle`对象可用，但与之前的示例不同的是，现在调用`popBack`会让主线程进入等待状态，并且只有在有新数据可用时才会恢复执行——从而显著减少了处理器的负载。
...
![DraggedImage](assets/DraggedImage-5889789.png)



### 练习：构建一个通用消息队列  

我们在交通仿真中用于管理`WaitingVehicles`的代码实际上是一个非常通用的代码片段。与其传递`Vehicle`对象，它可以很容易地修改为在两个`vector`之间传递几乎任何类型的对象或数据。因此，我们创建的内容可以很容易地描述为一个**通用消息队列**。  

为了将`WaitingVehicles`队列转换为一个通用消息队列，需要进行以下更改：  

> 1. 通过在类前添加`template<class T>`来启用模板，并将类名更改为`MessageQueue`。  
> 2. 将`std::vector`替换为`std::deque`，因为这更符合按“先进先出（FIFO）”顺序检索对象的逻辑。同时，将其重命名为`_messages`。  
> 3. 修改方法`pushBack`以支持模板，并将其重命名为`send`。对`popBack`方法进行类似的修改，并将其重命名为`receive`。  
> 4. 通过在`main`函数中执行以下代码来测试该队列：  
>
...
现在我们已经创建了一个通用版本的`MessageQueue`，它可以用来传递几乎任何数据类型。接下来，我们希望将这一概念应用到实际场景中。在即将到来的**最终项目**中，你将实现一个名为`TrafficLight`的新类，它负责管理十字路口的交通。而每个`TrafficLight`的基本机制将是基于我们在上一段代码示例中实现的`MessageQueue`的一个版本。
...
欢迎来到C++并发课程的最终项目。在过去几周里，你已经完成了从编写线性和同步代码到成为并发程序员的转变。现在你已经掌握了如何创建并行执行路径，以及如何在异步线程之间交换数据。

诸如`async`、`mutexes`、`locks`、`Lambdas`和`Rvalue reference`等概念，以及`move semantics`，现在都已成为你的工具箱的一部分，它们将大大提升你所编写代码的**质量**和**性能**。

在即将开始的项目中，你将通过一个新概念来扩展当前的交通模拟系统。除了现有的车辆、路口和街道之外，这座城市还需要**交通信号灯**来管理车流，而开发这个功能将是你的任务。让我们来看看具体内容。



## Tasks

让我们来看看C++并发课程最终项目中的任务。本次项目的目标是为**并发交通模拟**开发一个交通信号灯系统。目前的模拟程序运行良好，车辆能够按照预期在街道上行驶，依次通过路口，并且与路口安全通信，确保同一时刻只有一辆车可以进入路口。然而，随着交通流量的增加，为了保障**道路使用者的安全**，需要在城市网格的每个路口安装交通信号灯。

任务是开发一个适合的、**线程安全**的通信协议，用于车辆、路口和交通信号灯之间的通信。需要使用并发编程的知识，例如`mutexes`、`locks`以及特别是`message queues`，来实现交通信号灯，并将其正确集成到代码库中。

![Program Schematic for reference](assets/screenshot-2021-10-25-at-4.45.59-pm.png)

整个程序的示意图大部分内容你已经熟悉。

在左侧，我们有`main`函数，然后是方法`processVehicleQueue`和类`Intersection`，接着是管理车辆在城市网格中运动的`VehicleDrive`方法。第一个新变化出现在`Intersection::addVehicleToQueue`方法中，这里需要等待交通信号灯变绿。与之前相比，**主要的区别在于，即使路口允许车辆进入，如果信号灯是红色，车辆仍然需要等待直到信号灯变绿才能通行(`wait until entry has been granted by Intersection`)。**

在右侧，我们有一个全新的类`TrafficLight`和方法`cycleThroughPhases`。这是一个无限循环，初始化秒表(`init stop watch`)，计算时间差(`compute time difference to stop watch`)，然后在红绿之间切换状态(`toggle between red and green`)。每当达到适当的超时时间，信号灯会在红绿之间切换状态。此处仅有红绿两种状态，没有黄灯。一旦状态切换（无论是变为红灯还是绿灯），都会向队列发送更新消息(`send update message to queue`)。新的交通信号灯状态会被发送到相应的模块，在那里评估是红灯还是绿灯(`wait until TrafficLight is green`)，只有在绿灯时才会向车辆发送安全进入路口的信号。在`cycleThroughPhases`结束时，秒表会被重置(`reset stop watch`)，整个循环重新开始。

在实际运行中，你可以看到巴黎凯旋门广场的路口，交通信号灯的颜色在红绿之间切换。当信号灯变绿时，车辆会进入路口。如果车辆已经开始进入路口，即使信号灯变红，车辆也会继续通过。这种设计并非程序的错误，而是与现实生活一致：一旦车辆越过信号灯，即使信号灯变红，车辆仍然被允许离开路口。

接下来是具体的任务说明，总共有六个任务：

1. **定义类`TrafficLight`**  
    该类继承自`TrafficObject`，包含以下内容：  
    - 公共方法：`waitForGreen`、`simulate`和`TrafficLightPhase` ,`getCurrentPhase`  
    - 私有方法：`cycleThroughPhases`  
    - 私有成员：`_currentPhase`，表示当前信号灯的状态（红或绿）。
...
2. **实现一个无限循环函数**  
    ...
    - 使用**移动语义**向消息队列发送更新。  
    - 循环周期应为4到6秒的随机值。  
    - 循环间需要调用`std::this_thread::sleep_for`等待1毫秒，以降低处理器负载。
    - 最后私有方法`cycleThroughPhases` 启动一个线程当`simulate`被调用时，通过base class `TrafficObject`中的thread queue实现

3. **定义类`MessageQueue`**  
    该类包含以下内容：  
    - 公共方法：`send`和`receive`, `send` 方法应接受类型为 `TrafficLightPhase` 的右值引用，而 `receive` 方法应返回此类型。。  
    - 此外，该类应定义一个名为`_queue` 的 `std::dequeue`，用于存储类型为 `TrafficLightPhase` 的对象。以实现**先进先出**的队列。  
    - 私有成员：最后，该类还应有一个 `std::condition_variable` 和一个 `std::mutex` 作为私有成员。用于线程同步。

4. **实现方法`send`**  
    - 使用`std::lock_guard<std::mutex>`和`_condition.notify_one`将消息添加到队列，并发送通知。  
    - 在`TrafficLight`类中创建一个`MessageQueue`类型的私有成员，并在无限循环中调用`send`方法，将每个新的`TrafficLightPhase`对象推送到队列中。

5. **实现方法`receive`**  
    - 使用`std::unique_lock<std::mutex>`和`_condition.wait`等待并接收新消息，使用**移动语义**从队列中取出消息。  
    - 实现`waitForGreen`方法，其中包含一个无限循环，反复调用`receive`方法。当收到`TrafficLightPhase::green`时，方法返回。

6. **在类`Intersection`中集成交通信号灯功能**  
    - 添加一个私有成员`_trafficLight`，类型为`TrafficLight`。  
    - 在`Intersection::simulate`方法中启动交通信号灯的模拟。  
    - 在`Intersection::addVehicleToQueue`方法中，使用`TrafficLight::getCurrentPhase`和`TrafficLight::waitForGreen`方法阻塞执行，直到信号灯变绿。
